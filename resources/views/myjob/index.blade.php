    ...
    <section id="sec_condistions">
        <div class="container px-5">
            <div class="row flex-column flex-md-row justify-content-between gap-4 flex-wrap flex-xl-nowrap">
                <div class="col-lg-2">
                    <input class="form-control rounded-5 fs-14 h-100" type="text" placeholder="タイトルで検索する" id="search_title">
                </div>
                <div class="col-lg-2 position-relative">
                    <input name="" id="search_company" class="form-select select2 w-100 rounded-pill" placeholder="会社名による検索">
                ...
                <div class="col-lg-2 position-relative">
                    <input name="" id="search_owner" class="form-select select2 w-100 rounded-pill" placeholder="所有者名による検索">
                    <div class="select-cus position-absolute card p-3 shadow rounded-4">
                        <div class="cus-search">
                            <input type="text" name="search" class="form-control select-search" placeholder="検索...">
                        </div>
                        <div class="cus-options py-2">
                            <div class="cus-notfound"><span>見つかりません</span></div>
                            <div class="cus-option"><input type="checkbox" name="owner" value="{{ $user->id }}" id="admin"><label for="admin"><span>{{ $user->name }}</span></label></div>
                            @foreach ($owners as $item)
                            <div class="cus-option"><input type="checkbox" name="owner" value="{{ $item->user_id }}" id="{{ $item->id }}"><label for="{{ $item->id }}"><span>{{ $item->name }}</span></label></div>
                            @endforeach
                        </div>
                        <div class="btn-group">
                ...
                <div class="col-lg-2 position-relative">
                    <input name="" id="search_status" class="form-select select2 w-100 rounded-pill" placeholder="ステータスによる検索">
                    <div class="select-cus position-absolute card p-3 shadow rounded-4">
                        <div class="cus-search">
                            <input type="text" name="search" class="form-control select-search" placeholder="検索...">
                        </div>
                        <div class="cus-options py-2">
                            <div class="cus-notfound"><span>見つかりません</span></div>
                            <div class="cus-option"><input type="checkbox" name="status" value="draft" id="draft"><label for="draft"><span>草案</span></label></div>
                            <div class="cus-option"><input type="checkbox" name="status" value="live" id="live"><label for="live"><span>募集中</span></label></div>
                            <div class="cus-option"><input type="checkbox" name="status" value="finish" id="finish"><label for="finish"><span>募集完了</span></label></div>
                            <div class="cus-option"><input type="checkbox" name="status" value="closed" id="closed"><label for="closed"><span>クローズド</span></label></div>
                        </div>
                        <div class="btn-group">
            ...
            <table class="table table-bordered mt-3 mt-xl-5">
                ...
                <tbody id="tbody">
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <a href="@if ($item['status'] == 'live') {{ route('getJobDetail', ['url' => $item['url']]) }}@else javascript:; @endif" @if ($item['status'] !='live' ) style="visibility: hidden; cursor: pointer;" @endif class="me-3">
